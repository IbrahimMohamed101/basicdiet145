...
function buildPhase1SubscriptionContract({ payload = {}, resolvedQuote, actorContext = {}, source, now = new Date(), renewalSeed = null, currentBusinessDate = null } = {}) {
  ...
  const premiumItems = (resolvedQuote.premiumItems || []).map((item) => ({ proteinId: String(item.protein && item.protein._id ? item.protein._id : item.proteinId || ""), qty: Number(item.qty || 0), unitExtraFeeHalala: Number(item.unitExtraFeeHalala || 0), currency: String(item.currency || "SAR") }));
