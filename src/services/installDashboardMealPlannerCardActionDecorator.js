...
function optionRole(section = {}) {
  ...
  if (explicit) return explicit;
  if (sectionKey(section) === "carbs" || ["carb", "carbs"].includes(token(section.metadata?.sourceGroupKey))) {
    return "carbs";
  }
  return "protein";
}
...
function decorateSection(section) {
  ...
  const role = resolvedType === CARD_TYPES.OPTION_FAMILY ? optionRole(section) : null;
  return {
    ...section,
...
async function decorateResult(result) {
  ...
  return {
    ...
    summary: {
      ...(result.summary || {}),
      sectionCount: draft?.sections?.length ?? result.summary?.sectionCount ?? 0,
      selectedProductCount:
        draft?.sections?.reduce((sum, item) => sum + productIds(item).length, 0) ??
        result.summary?.selectedProductCount ??
        0,
      selectedOptionCount:
        draft?.sections?.reduce((sum, item) => sum + optionIds(item).length, 0) ??
...
function wrap(methodName) {
  ...
  if (typeof original !== "function" || original.__mealPlannerCardActionDecorator) return;
  const decorated = async function decoratedCardAction(args = {}) {
    return decorateResult(await original.call(mealBuilderService, args));
  };
...
module.exports = { installDashboardMealPlannerCardActionDecorator };
