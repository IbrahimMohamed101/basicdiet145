...
async function listPlans(req, res) {
  ...
  const rawPlans = await Plan.find(Plan.getSellableQuery())
  ...
  for (const plan of rawPlans) {
    if (Plan.isViable(plan)) {
      ...
      logger.warn(
        `Plan ${plan._id} is sellable but non-viable (missing/inactive options). Skipping in public catalog.`,
        {
          planId: plan._id,
          name: plan.name,
          key: plan.key || null,
...
async function getPlan(req, res) {
  ...
  if (!Plan.isViable(plan)) {
    logger.warn(`Plan ${plan._id} requested directly but is non-viable.`, {
      planId: plan._id,
      key: plan.key || null,
    });
  ...
  return res.status(200).json({
    status: true,
    data: resolvePlanCatalogEntry(plan, lang),
  });
}
