...
function run() {
  ...
  assert.deepStrictEqual(
    LEGACY_STAFF_ROLES,
    ["kitchen", "cashier"],
    "legacy operational roles remain identifiable during migration"
  );
  assert(
    STAFF_FILTER_ROLES.includes("kitchen") && STAFF_FILTER_ROLES.includes("cashier"),
    "legacy users remain listable and filterable before migration"
  );
