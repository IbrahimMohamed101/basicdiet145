...
(async function main() {
  ...
  const adminServiceDate = resolveFirstServiceDate({
    requestedStartDate: new Date("2026-07-21T00:00:00+03:00"),
    currentBusinessDate: "2026-07-21",
    rootDeliveryType: normalized.type,
    firstDayPickupOverride: null,
    allowSameDayDeliveryStart: true,
  });
  assert.strictEqual(adminServiceDate.resolvedDate, "2026-07-21");
  assert.strictEqual(adminServiceDate.shifted, false);
  assert.strictEqual(adminServiceDate.fulfillmentOptions.sameDayDeliveryAllowed, true);
  assert.strictEqual(adminServiceDate.fulfillmentOptions.reason, null);
