...
function parseArgs(argv = process.argv.slice(2)) {
  return {
    sync: argv.includes("--sync") || isTruthy(process.env.ACCOUNT_BOOTSTRAP_SYNC),
  };
}
...
async function createDashboardAccount(account, { sync = false } = {}) {
  ...
  if (existing) {
    if (sync) {
      ...
      const updated = await DashboardUser.findById(existing._id);
      return {
        type: "dashboard",
        status: "updated",
        ...
        reason: "ACCOUNT_BOOTSTRAP_SYNC enabled",
        user: updated,
...
async function createAppAccount(account, { sync = false } = {}) {
  ...
  if (existingCoreUser || existingAppUser) {
    if (sync && existingCoreUser) {
      ...
      const updatedCoreUser = await User.findById(existingCoreUser._id);
        ...
        await AppUser.updateOne(
          { _id: appUser._id },
          {
            $set: {
              ...
              coreUserId: updatedCoreUser._id,
            },
          },
          { runValidators: true }
        );
        appUser = await AppUser.findById(appUser._id);
      } else {
...
async function bootstrapDefaultAccounts({ sync = false } = {}) {
  assertProductionAllowed();
  await connectSafely();
  console.log(`[accounts-bootstrap] Mode: ${sync ? "sync" : "create-missing-only"}`);

  const results = [];
  for (const account of DASHBOARD_ACCOUNTS) {
    const result = await createDashboardAccount(account, { sync });
    results.push(result);
    printAccountResult(result);
  }

  for (const account of APP_ACCOUNTS) {
    const result = await createAppAccount(account, { sync });
    results.push(result);
    printAccountResult(result);
  }
  ...
  console.log("[summary] created=%d updated=%d skipped=%d verificationWarnings=%d", created.length, updated.length, skipped.length, failedVerification.length);
  if (sync && failedVerification.length > 0) {
    throw new Error("Account bootstrap sync failed verification");
  }
  return { results, created, updated, skipped, failedVerification };
}

async function main() {
  try {
    const args = parseArgs();
    const summary = await bootstrapDefaultAccounts({ sync: args.sync });
